...
#define vidc_in_range(val, min, max) (((min) <= (val)) && ((val) <= (max)))
...
static bool msm_vidc_allow_image_encode_session(struct msm_vidc_inst *inst)
{
	...
	fmt = &inst->fmts[INPUT_PORT];
	if (!vidc_in_range(fmt->fmt.pix_mp.width, min_width, max_width) ||
		!vidc_in_range(fmt->fmt.pix_mp.height, min_height, max_height)) {
...
static int msm_vidc_check_resolution_supported(struct msm_vidc_inst *inst)
{
	...
	if (is_decode_session(inst) || is_encode_session(inst)) {
		if (!vidc_in_range(width, min_width, max_width) ||
			!vidc_in_range(height, min_height, max_height)) {
			i_vpr_e(inst,
